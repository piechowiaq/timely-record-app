...
<template>
    <div class="block whitespace-nowrap">
        <div v-if="$page.props.flash.success && show" class="sm:mt-0 mt-2 flex items-center justify-between p-1 bg-green-500 rounded">
            <div class="flex items-center">
                <svg class="flex-shrink-0 ml-4 mr-2 w-2 h-2 fill-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><polygon points="0 11 2 9 7 14 18 3 20 5 7 18" /></svg>
                <div class="text-white text-xs font-medium">{{ $page.props.flash.success }}</div>
            </div>
        ...
        <div v-if="($page.props.flash.error || Object.keys($page.props.errors).length > 0) && show" class="sm:mt-0 mt-2 flex items-center justify-between p-1 bg-red-500 rounded">
            <div class="flex items-center">
                <svg class="flex-shrink-0 ml-4 mr-2 w-2 h-2 fill-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M2.93 17.07A10 10 0 1 1 17.07 2.93 10 10 0 0 1 2.93 17.07zm1.41-1.41A8 8 0 1 0 15.66 4.34 8 8 0 0 0 4.34 15.66zm9.9-8.49L11.41 10l2.83 2.83-1.41 1.41L10 11.41l-2.83 2.83-1.41-1.41L8.59 10 5.76 7.17l1.41-1.41L10 8.59l2.83-2.83 1.41 1.41z" /></svg>
                <div v-if="$page.props.flash.error" class="text-white text-xs font-medium">{{ $page.props.flash.error }}</div>
                <div v-else class="text-white text-xs font-medium">
