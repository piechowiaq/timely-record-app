...
const getWorkspaceBorderColor = (workspace) => {
  if (workspace.percentageOfUpToDate === 100) return 'border-green-500';
  if (workspace.percentageOfUpToDate > 90) return 'border-green-300';
  if (workspace.percentageOfUpToDate >= 80) return 'border-yellow-300';
        ...
        <section v-for="workspace in workspaces" :key="workspace.id"
          ...
          <section class="grid grid-cols-2 gap-4 text-center">
            <div :class="`border-2 p-2 flex flex-col ${getWorkspaceBorderColor(workspace)}`">
              ...
              <p class="font-medium text-2xl text-gray-500 mt-auto">{{ workspace.percentageOfUpToDate }}%</p>
